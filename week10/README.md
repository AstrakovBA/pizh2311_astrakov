...
**Этот проект:**
- Использует массив из 64 элементов uint32_t (256 байт + небольшие накладные расходы для избежания переполнения)
- Поддерживает все требуемые операции
- Корректно обрабатывает переполнения
- Имеет компактный размер (менее 300 байт)

**Особенности реализации:**
- Деление реализовано методом дихотомии (т. е. длинного деления, когда перед операцией число разбивается на равные промежутки)
